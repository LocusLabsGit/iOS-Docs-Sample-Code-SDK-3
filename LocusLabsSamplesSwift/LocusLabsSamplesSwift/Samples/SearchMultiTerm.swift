...
class SearchMultiTerm: UIViewController, LLVenueDatabaseDelegate, LLMapViewDelegate, LLSearchDelegate {
    ...
    func search(_ search: LLSearch!, multiTermSearchResults searchResults: LLMultiTermSearchResults!) {
        ...
        print("result count", searchResults.results.count)
        
        for searchResult in searchResults.results as! [LLSearchResult] {
            
            let position = searchResult.position
            createCircle(position: position!, radius: 10, color: .green)
        }
    }
